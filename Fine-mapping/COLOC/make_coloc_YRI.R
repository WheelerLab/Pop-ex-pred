...
for(pop in 1:length(pops)){ #read in pop's .frq file for MAF
  ...
  for(pheno in phenos){ #read in GEMMA output file
    GEMMA_result <- fread("/home/elyse/Wojcik_build37/31217584-GCST008053-EFO_0004339-build37.f.tsv.gz", header = T)
    ...
    GEMMA_for_COLOC$sample_size <- 49796
    ...
    fwrite(eQTL_write, "/home/elyse/coloc/Sig_eQTL_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt", quote = F, sep = "\t", na = "NA", row.names = F, col.names = T)
    gzip("/home/elyse/coloc/Sig_eQTL_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt", destname = "/home/elyse/coloc/Sig_Height_eQTL_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt.gz") #script may only take .gz values so can't hurt to be too careful
    fwrite(GWAS_write, "/home/elyse/coloc/Sig_GWAS_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt", row.names = F, col.names = T, sep = "\t", quote = F, na = "NA")
    gzip("/home/elyse/coloc/Sig_GWAS_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt", "/home/elyse/coloc/Sig_Height_GWAS_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt.gz")
