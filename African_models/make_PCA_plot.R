...
pcs <- read.table("/home/egeoffroy/AFR_PCA.txt", header =F)
...
p <- ggplot() + geom_point(data=gwas,aes(x=PC1,y=PC2,col=population,shape=population))+geom_point(data=hm3,aes(x=PC1,y=PC2,col=population,shape=population))+ theme_bw() + scale_colour_brewer(palette="Set1")
