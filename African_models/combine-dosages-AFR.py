...
for chr in chrs:
    #load in first
    pop_1 = pd.read_csv("/home/hwheeler1/from_wheelerlab3_2019-08-29/jennifer/New_Dosages/" + pops[0] + "/" + pops[0] + "_" + str(chr) + ".dosage.txt", delim_whitespace = True, header = None)
    pop_1.columns = ["chr", "rsid", "pos", "A1", "A0", "AF"] + list(pd.read_csv("/home/hwheeler1/from_wheelerlab3_2019-08-29/jennifer/samples_" + pops[0] + ".txt", delim_whitespace = True, header = None).loc[:, 0]) #add column names of individuals to keep them straight
    ...
    for pop_num in range(1, len(pops)):
        pop_2 = pd.read_csv("/home/hwheeler1/from_wheelerlab3_2019-08-29/jennifer/New_Dosages/" + pops[pop_num] + "/" + pops[pop_num] + "_" + str(chr) + ".dosage.txt", delim_whitespace = True, header = None)
        pop_2.columns = ["chr", "rsid", "pos", "A1", "A0", "AF"] + list(pd.read_csv("/home/hwheeler1/from_wheelerlab3_2019-08-29/jennifer/samples_" + pops[pop_num] + ".txt", delim_whitespace = True, header = None).loc[:, 0])
...
samples = list(pop_1.columns[6:])
exp_1 = pd.read_csv("/home/hwheeler1/from_wheelerlab3_2019-08-29/jennifer/hapmap_expression_ensid/" + pops[0] + "_expression_ens.txt", delim_whitespace = True, index_col = 0)
for pop_num in range(1, len(pops)):
    exp_2 = pd.read_csv("/home/hwheeler1/from_wheelerlab3_2019-08-29/jennifer/hapmap_expression_ensid/" + pops[pop_num] + "_expression_ens.txt", delim_whitespace = True, index_col = 0)
